          ...
          <AnimatePresence mode="wait">
            {!role ? (
              ...
              <motion.div key="role"
                ...
                <p className="text-muted-foreground text-sm mb-6">Choisis ton profil pour commencer.</p>

                {/* Google OAuth */}
                <button
                  type="button"
                  onClick={() => supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${window.location.origin}/connexion` } })}
                  className="w-full flex items-center justify-center gap-3 px-5 py-3 rounded-full border-2 border-border bg-white text-foreground text-sm font-bold hover:border-primary transition-colors mb-4"
                >
